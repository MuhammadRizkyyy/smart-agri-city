...
# 🚀 FIX: Load the models immediately at the global level so pytest can see them instantly!
...
if os.path.exists(model_path):
    ...
    models_pack = None
    print("⚠️ Warning: Model artifact file not found yet.")
...
def predict_pest(data: PestInput):
    ...
    zone_encoded = models_pack["encoders"]["zone"].transform([data.zone])
...
def predict_irrigation(data: IrrigationInput):
    ...
    growth_encoded = models_pack["encoders"]["growth_phase"].transform([data.growth_phase])
    input_dict = data.model_dump()
    ...
    if data.soil_moisture < 25: urgency = "Kritis"
    elif data.soil_moisture < 50: urgency = "Segera"
    else: urgency = "Tidak Perlu"
