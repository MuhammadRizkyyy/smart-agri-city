...
beforeAll(async () => {
  ...
  stubs.irrigation = await startStub(19003, (req, res) => {
    ...
    if (req.url === "/health") {
      ...
      res.writeHead(200);
      res.end(
        JSON.stringify({ status: "success", message: "sensor received" }),
      );
    } else {
      res.writeHead(404);
      res.end("{}");
...
afterAll(async () => {
  await Promise.all(
    Object.values(stubs).map((s) => new Promise((r) => s.close(r))),
  );
});
...
const makeValidJWT = () =>
  jwt.sign({ sub: "1", role: "farmer" }, process.env.JWT_SECRET, {
    expiresIn: "1h",
  });
...
describe("GET /api/farmers proxy", () => {
  ...
  test("JWT valid → di-forward ke FARMER_SERVICE_URL dengan header Authorization", async () => {
    ...
    postSpy = jest.spyOn(axios, "post").mockResolvedValueOnce({
      data: { active: true, sub: "1", scope: "read" },
    });
  ...
  test("JWT expired/inactive → gateway return 401", async () => {
    const expiredToken = jwt.sign({ sub: "1" }, process.env.JWT_SECRET, {
      expiresIn: "-1s",
    });
    // oauthIntrospect memanggil axios.post — mock supaya return active:false (token revoked/expired)
    postSpy = jest.spyOn(axios, "post").mockResolvedValueOnce({
      data: { active: false },
    });
    ...
    expect(res.statusCode).toBe(401);
    expect(res.body.message).toMatch(/inactive|revoked|expired|unauthorized/i);
  });
});
...
describe("POST /iot/sensor — IoT route (OAuth client_credentials)", () => {
  ...
  test("tanpa Authorization header → return 401", async () => {
    const res = await request(app).post("/iot/sensor").send({ zone: "zona1" });
