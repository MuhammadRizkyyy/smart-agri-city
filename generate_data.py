...
print("⏳ Generating highly predictive smart agriculture datasets...")

# =====================================================================
# 1. CROP YIELD DATASET (Tuned for R2 >= 0.70)
# =====================================================================
n_yield = 2200
...
# Stronger, cleaner mathematical signal for Random Forest to capture easily
base_yield = (
    (soil_moisture * 0.4)
    + (nitrogen * 0.5)
    + (phosphorus * 0.3)
...
# Reduce the noise factor slightly
...
# =====================================================================
# 2. PEST AND DISEASE DATASET (Tuned for Accuracy >= 70%)
# =====================================================================
n_pest = 1600
...
for i in range(n_pest):
    # Highly distinct logical boundaries for much cleaner classification
    if air_humidity[i] > 83 and leaf_temp[i] < 25.5:
        prob = [0.02, 0.03, 0.90, 0.03, 0.02]  # Pure Blast Fungus conditions
    elif air_humidity[i] > 80 and leaf_temp[i] > 31.5:
        prob = [0.02, 0.90, 0.03, 0.03, 0.02]  # Pure Wereng conditions
    elif chlorophyll[i] < 36 and soil_ph[i] < 5.8:
        prob = [0.03, 0.02, 0.02, 0.85, 0.08]  # Pure Tungro conditions
    elif chlorophyll[i] < 39 and soil_ph[i] > 7.2:
        prob = [0.03, 0.02, 0.02, 0.08, 0.85]  # Pure Layu_Fusarium conditions
    else:
        prob = [0.92, 0.02, 0.02, 0.02, 0.02]  # Pure Healthy conditions
...
# =====================================================================
# 3. IRRIGATION DEMAND DATASET (Maintained High Accuracy)
# =====================================================================
n_irrig = 1600
...
df_irrig = pd.DataFrame({
    ...
    "growth_phase": growth_phase, "evapotranspiration": evapo, "water_needed_liters": water_needed, "irrigation_urgency": urgency
})
