...
print(f"Yield R2 Score : {r2_yield:.4f}  (target >= 0.70)")
print(f"5-Fold CV Mean : {cv_yield.mean():.4f}")


# 2. PEST & DISEASE CLASSIFIER
...
print(f"Pest Accuracy  : {acc_pest * 100:.2f}%  (target >= 70%)")
print(f"5-Fold CV Mean : {cv_pest.mean() * 100:.2f}%")


# 3. IRRIGATION DEMAND OPTIMIZER
# Algoritma: GradientBoostingRegressor sesuai PLAN.md Section 6.1.
# GradientBoostingRegressor tidak support multi-output secara native,
# sehingga dibungkus dengan MultiOutputRegressor yang melatih satu estimator
# per target (water_needed_liters dan irrigation_urgency) secara independen.
print("\n--- Irrigation Demand Optimizer ---")
...
model_irrig = MultiOutputRegressor(
    GradientBoostingRegressor(n_estimators=150, learning_rate=0.05, random_state=42)
)
model_irrig.fit(X_train_i, y_train_i)

cv_irrig = cross_val_score(model_irrig, X_irrig, y_irrig, cv=5, scoring="r2")
r2_irrig = model_irrig.score(X_test_i, y_test_i)
print(f"Irrigation R2  : {r2_irrig:.4f}  (target >= 0.70)")
print(f"5-Fold CV Mean : {cv_irrig.mean():.4f}")


# SAVE ALL MODELS TO A SINGLE ARTIFACT
model_payload = {
...
print("\nAll models saved to models/agri_models.pkl")
